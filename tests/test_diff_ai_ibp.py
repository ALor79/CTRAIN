"""
Trains CNN7_Shi on MNIST using DiffAI with IBP bounds (baseline).
"""
...
wrapper.eval()
# IBP falls back to super().evaluate() which uses CROWN for certified accuracy.
std_acc, cert_acc, _ = wrapper.evaluate(test_loader=test_loader, test_samples=1000)

print(f"\n{'='*60}")
print(f"  IBP (baseline) — {NUM_EPOCHS} epochs, eps={EPS}, MNIST")
print(f"{'='*60}")
